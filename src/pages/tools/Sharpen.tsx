...
const Sharpen = () => {
  ...
  const handleSharpen = async () => {
    ...
    try {
      ...
      addToast({
        title: 'Image sharpened successfully',
        type: 'success'
      });
    } catch (error) {
      setStatus('error');
  ...
  return (
    <ToolLayout
      ...
      description="Enhance image clarity and detail with precise sharpening controls"
    >
      <div className="space-y-8">
        ...
        {status === 'processing' && (
          <ProcessingStatus 
            status="processing" 
            progress={progress}
            message="Applying sharpening effect..."
          />
        )}
        
        {status === 'completed' && resultUrl && (
          <div className="text-center space-y-4">
            <p className="text-green-500 mb-4">Image sharpened successfully!</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="border border-gray-800 rounded-lg overflow-hidden">
                <h4 className="text-sm font-medium p-2 bg-gray-800">Sharpened</h4>
                <img 
                  src={resultUrl} 
                  alt="Sharpened" 
                  className="w-full"
